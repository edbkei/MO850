...
def make_statistics(arg_in): 
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Energy(J)')
    plt.title('Energy Consumed by Wireless node')
    #fig = plt.figure()
    #ax = plt.subplot(111)
    ...
    aux2="Pkts Received by dest. Node="+str(k[1])
    aux3="Distance between Nodes="+str(k[2])
    plt.figtext(.5, .8, aux1, fontsize='small')
    plt.figtext(.5, .75, aux2, fontsize='small')
    plt.figtext(.5, .7, aux3, fontsize='small')
